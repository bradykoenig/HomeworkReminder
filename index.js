...
app.listen(PORT, '0.0.0.0', () => console.log(`Keep-alive server running on port ${PORT}`))
   .on('error', (err) => {
       if (err.code === 'EADDRINUSE') {
           console.error(`Port ${PORT} is already in use. Attempting to use a random port.`);
           const randomPort = Math.floor(Math.random() * 64512) + 1024; // Generate a random port
           app.listen(randomPort, '0.0.0.0', () => console.log(`Keep-alive server running on random port ${randomPort}`));
       } else {
           throw err;
       }
   });
...
client.on('interactionCreate', async interaction => {
    ...
    if (commandName === 'create-announcement') {
        ...
        await interaction.reply(`Homework "${title}" has been added with a deadline of ${dueDate}.`);
    }
...
function checkDeadlines() {
    ...
    for (const [title, deadline] of Object.entries(homeworkDeadlines)) {
        ...
        if (!sentReminders[deadlineKey]) {
            if (moment(deadline).isSameOrBefore(moment(now).add(1, 'days')) && moment(deadline).isAfter(now)) {
                channel.send(`⏰ Reminder: Homework "${title}" is due tomorrow!`);
